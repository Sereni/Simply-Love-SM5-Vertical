...
-- Let's see if we're okay with being judged
if SL[pn].ActiveModifiers.DoNotJudgeMe then return end

-- ---------------------------------------------------------------
-- Make sure that someone requested *something* from this file.
-- There are four reasons we'd want to proceed further.
local WantsPacemaker        = SL[pn].ActiveModifiers.Pacemaker
-- TODO: use_smaller_graph is a thing, does it fit in Vertical? ~Roujo
local WantsTargetGraph      = false -- SL[pn].ActiveModifiers.DataVisualizations == "Target Score Graph"
...
local isTwoPlayers = (GAMESTATE:IsPlayerEnabled(PLAYER_1) and GAMESTATE:IsPlayerEnabled(PLAYER_2))false
